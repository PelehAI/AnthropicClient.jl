...
#
# Anthropic enforces requests-per-minute caps per API key. Many simultaneous
# tasks must share one rate budget. We use a sliding-window approach: we keep
# a deque of timestamps of recent calls; before issuing a new call we evict
# entries older than 60s and block if the deque is at the cap.
